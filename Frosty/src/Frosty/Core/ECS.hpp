...
#pragma region Component
		...
		struct CWeapon : public BaseComponent
		{
			...
			enum class WeaponType { Sword, Arrow, Bite };
			...
			CWeapon() = default;
			CWeapon(WeaponType type, unsigned int itemID, int damage, bool isPlayerWeapon = false) : Type(type), ItemID(itemID), Damage(damage), IsPlayerWeapon(isPlayerWeapon) { }
		...
		struct CAttack : public BaseComponent
		{
			...
			CAttack() = default;
			CAttack(int damage, bool friendly = false, float lifeTime = 0.5f, bool destroyable = true) : Damage(damage), Friendly(friendly), Lifetime(lifeTime), Destroyable(destroyable) { AttackedEntities.reserve(5); }

...
#pragma region Component
		...
		struct BaseComponent
		{
			...
			BaseComponent& operator=(const BaseComponent& org)
			{
				if (this != &org)
				{
					EntityPtr = org.EntityPtr;
				}
				return *this;
			}
		...
		struct CWeapon : public BaseComponent
		{
			...
			enum class WeaponType { Sword, Arrow, Fang };	// Humanoid enemies use sword and projectiles whereas wolves use fang attacks
			WeaponType Type{ WeaponType::Sword };

			std::string	WeaponName{ "1Default" };			// Used to defferinciate each kind of weapon
			...
			CWeapon() = default;
			CWeapon(WeaponType type, float damage) : Type(type), Damage(damage) { }
			CWeapon(WeaponType type, std::string weaponName, float damage, float criticalHitChance) : Type(type), WeaponName(weaponName), Damage(damage), CriticalHitChance(criticalHitChance) { }
			CWeapon(WeaponType type, float damage, float criticalHit, float criticalHitChance, float cooldown, float lifetime = 3.f) : Type(type), Damage(damage), CriticalHit(criticalHit), CriticalHitChance(criticalHitChance), Cooldown(cooldown), Lifetime(lifetime) { }
			...
			bool operator!=(const CWeapon& org) { return WeaponName != org.WeaponName; }
		...
		struct CPlayer : public BaseComponent
		{
			...
			CPlayer& operator=(const CPlayer& org)
			{
				...
				if (this != &org)
				{
					...
					SpeedBootsKey = org.SpeedBootsKey;
		...
		struct CEnemy : public BaseComponent
		{
			...
			{
				FY_WARN("Assignment operator in CEnemy called.");
		...
		struct CParticleSystem : public BaseComponent
		{
			struct Particle
			{
				...
				Particle(glm::vec4 color)
					: Color(color) {};
				Particle& operator=(const Particle& org)
				{ 
					if (this != &org)
					{
						Position = org.Position;
						...
						Direction = org.Direction;
						StartPos = org.StartPos;
						Lifetime = org.Lifetime;
						Speed = org.Speed;
						StartSize = org.StartSize;
						EndSize = org.EndSize;

						CamDistance = org.CamDistance;
					}
					return *this;
				}
			...
			struct GPUParticle
			{
				...
				GPUParticle& operator=(const GPUParticle& org)
				{
					if (this != &org)
					{
						Position = org.Position;
			...
			{
				FY_WARN("Assignment operator in CParticleSystem called.");
				...
				if (this != &org)
				{
					...
					{
						Particles[i] = org.Particles[i];
					}
					for (size_t i = 0; i < MAX_PARTICLE_COUNT; i++)
					{
						GpuParticles[i] = org.GpuParticles[i];
					}

					ParticleVertArray = org.ParticleVertArray;
					UseShader = org.UseShader;
					ComputeShader = org.ComputeShader;
					Texture = org.Texture;
				}
				return *this;
			}

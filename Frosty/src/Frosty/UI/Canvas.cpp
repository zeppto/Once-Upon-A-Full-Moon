...
namespace Frosty
{
	...
	void Canvas::LoadMesh()
	{
		//auto& test = AssetManager::GetMesh("quad");
		//m_vertexArray = AssetManager::GetMesh("quad");
		//vertexArray.reset(FY_NEW VertexArray);
		//vertexArray = std::make_shared<VertexArray>(AssetManager::GetMesh("quad"));
		//vertexArray.reset(FY_NEW VertexArray(AssetManager::GetMesh("quad")));
	}

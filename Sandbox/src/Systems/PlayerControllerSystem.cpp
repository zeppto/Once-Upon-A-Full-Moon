...
namespace MCS
{
	...
	void PlayerControllerSystem::CreateLVL1BoundingBox(const std::shared_ptr<Frosty::ECS::Entity>& weaponCarrier, const std::shared_ptr<Frosty::ECS::Entity>& weapon)
	{
		...
		m_World->AddComponent<Frosty::ECS::CAttack>(sword, (int)glm::round(weaponComp.Damage + criticalHit), true); // <-- true in the end because it's a friendly attack
	}
	...
	void PlayerControllerSystem::CreateLVL1Projectile(const std::shared_ptr<Frosty::ECS::Entity>& weaponCarrier, const std::shared_ptr<Frosty::ECS::Entity>& weapon)
	{
		...
		float criticalHit = 0;
		criticalHit = GenerateCriticalHit(weaponComp.CriticalHit, weaponComp.CriticalHitChance);
		m_World->AddComponent<Frosty::ECS::CAttack>(projectile, (int)glm::round(weaponComp.Damage + criticalHit), true, weaponComp.Lifetime);
	}
	...
	void PlayerControllerSystem::CreateLVL3Projectile(const std::shared_ptr<Frosty::ECS::Entity>& weaponCarrier, const std::shared_ptr<Frosty::ECS::Entity>& weapon)
	{
		...
		float criticalHit = 0;
		criticalHit = GenerateCriticalHit(weaponComp.CriticalHit, weaponComp.CriticalHitChance);
		m_World->AddComponent<Frosty::ECS::CAttack>(projectile, (int)glm::round(weaponComp.Damage + criticalHit), true, weaponComp.Lifetime, false);
	}

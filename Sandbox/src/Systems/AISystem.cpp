...
namespace MCS
{
	...
	void AISystem::HandleAttack(size_t index)
	{
		...
		if (m_Enemy[index]->AttackDelay >= 1.0f)
		{
			...
			else if (m_Enemy[index]->Weapon->Type == Frosty::ECS::CWeapon::WeaponType::Sword)
			{
				...
				m_World->AddComponent<Frosty::ECS::CMaterial>(attack, materialHandler->GetMaterialByName("default"));			// Remove later
	...
	void AISystem::SpawnBoss()
	{
		...
		auto& bossMat = m_World->AddComponent<Frosty::ECS::CMaterial>(boss, materialHandler->GetMaterialByName("boss"));

		...
		if (existingFile.good())
		{
			...
			for (int i = 0; i < heder.NrOfEntitys; i++)
			{
				...
				if (fileEntitys.myEntitys[i].MyComponents[0].HaveComponent)
				{
					...
					{
						fileEntitys.myEntitys[i].myTransform.Scale.x *= 1.25;
							fileEntitys.myEntitys[i].myTransform.Scale.z *= 1.25;
					}

					auto& entity = m_World->CreateEntity(glm::vec3(matrix[3].x, matrix[3].y, matrix[3].z), tempRotation, fileEntitys.myEntitys[i].myTransform.Scale, fileEntitys.myEntitys[i].myTransform.IsStatic);
					...
					{
						existingFile.read((char*)& fileEntitys.myEntitys[i].myLevelExit, sizeof(Level_LevelExit));
						int newExit = fileEntitys.myEntitys[i].myLevelExit.ExitDirection;
